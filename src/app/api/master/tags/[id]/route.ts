...
export async function GET(_request: Request, { params }: RouteContext) {
  ...
  try {
    ...
    return NextResponse.json({ error: `Failed to fetch tag ${id}` }, { status: 500 });
  }
}
  ...
  try {
    ...
    if (error instanceof Prisma.PrismaClientKnownRequestError) {
      if (error.code === 'P2002') {
        ...
        if (constraintTargetIncludes(target, 'name')) {
          return NextResponse.json({ error: 'Tag with this name already exists' }, { status: 409 });
        }
        if (constraintTargetIncludes(target, 'slug')) {
          return NextResponse.json({ error: 'Tag with this slug already exists' }, { status: 409 });
        }
        return NextResponse.json(
          { error: 'Tag already exists due to unique constraint' },
          { status: 409 },
        );
      }

      if (error.code === 'P2025') {
        return NextResponse.json({ error: `Tag with id ${id} not found` }, { status: 404 });
      }
    }

    return NextResponse.json({ error: `Failed to update tag ${id}` }, { status: 500 });
  }
}
...
export async function DELETE(_request: Request, { params }: RouteContext) {
  ...
  try {
    ...
    return NextResponse.json({ error: `Failed to delete tag ${id}` }, { status: 500 });
  }
}
